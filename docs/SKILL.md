...
1. **Edit the `src/` tree. Never hand-edit `app.js`** — it is generated by
   `src/build.js` (concatenates `src/core/*.mjs` + the `.jsx`). Rebuild with
   `node src/build.js`.
2. **Pure logic → `src/core/*.mjs`** (no React/DOM/timers), unit-tested via
   `node --test`. App UI stays in the `.jsx` with `export default function
   App()`. No external CSS — styles live in the `CSS` string.
